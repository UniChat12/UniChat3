...
async function ensureMember(userId: string, chatId: string): Promise<boolean> {
  ...
  app.get("/v1/chats", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    const chats = members.map((m) => {
      ...
      const peerDisplayName =
        `${peer?.firstName ?? ""} ${peer?.lastName ?? ""}`.trim() || peer?.displayName || peer?.phone;
    ...
    const chat = await prisma.chat.create({
      data: {
        ...
        members: {
          create: [
            { userId: request.auth.uid, role: ChatRole.OWNER },
            ...uniqueMembers.map((userId) => ({ userId, role: ChatRole.MEMBER as ChatRole }))
          ]
        }
      }
    });
